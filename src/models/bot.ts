...
export class Bot {
    ...
    private onShardReady(shardId: number, _unavailableGuilds: Set<Snowflake> | undefined): void {
    ...
    private async onMessage(msg: Message): Promise<void> {
        ...
        try {
            let filledMessage = await PartialUtils.fillMessage(msg);
            if (!filledMessage) {
                return;
            }
    ...
    private async onReaction(
        ...
        try {
            let filledReaction = await PartialUtils.fillReaction(msgReaction);
            if (!filledReaction) {
                return;
            }

            let filledUser = await PartialUtils.fillUser(reactor);
            if (!filledUser) {
                return;
            }

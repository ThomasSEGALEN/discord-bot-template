...
if (!CLIENT_ID) {
    Logger.error('CLIENT_ID is not defined in environment variables.');
    process.exit(1);
}
    ...
    public async process(
        ...
        switch (args[3]) {
            ...
            case 'register': {
                if (localCmdsOnly.length > 0) {
                    ...
                    for (let localCmd of localCmdsOnly) {
                        await this.rest.post(Routes.applicationCommands(CLIENT_ID), {
                            body: localCmd,
                        });
                    }
            ...
            case 'rename': {
                ...
                await this.rest.patch(Routes.applicationCommand(CLIENT_ID, remoteCmd.id), {
                    body,
                });
            ...
            case 'delete': {
                ...
                await this.rest.delete(Routes.applicationCommand(CLIENT_ID, remoteCmd.id));
